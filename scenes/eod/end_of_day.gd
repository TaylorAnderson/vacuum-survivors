...
func init(invoice_data:Game.InvoiceData):
	...
	tween.tween_property(title, "position", Vector2(title.position.x - title_fadein_move_amt, title.position.y), 0.5);
	tween.tween_property(title, "modulate", Color.WHITE, 0.5);
	await get_tree().create_timer(1).timeout;
	var tween2 = create_tween();
	tween2.set_pause_mode(Tween.TWEEN_PAUSE_PROCESS);
	tween2.set_parallel(true);
	tween2.tween_property(total_line, "position", total_line.position + Vector2.UP * 20, 0.3);
	tween2.tween_property(total_line, "modulate", Color.WHITE, 0.3)
	tween2.tween_property(h_separator, "position", h_separator.position + Vector2.UP * 20, 0.3);
	tween2.tween_property(h_separator, "modulate", Color.WHITE, 0.3);
	await get_tree().create_timer(0.5).timeout;
	
	print(invoice_data.dust_remaining);
	var total_amount = 0;
...
func _on_shop_button_pressed():
	Events.change_scene.emit(MainScene.State.SHOP);
