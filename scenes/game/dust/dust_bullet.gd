...
func _process(delta):
	if sprite_2d.scale.x > 1: sprite_2d.scale *= 0.7;
	if sprite_2d.scale.y < 2: sprite_2d.scale.y *= 1.1
	position += velocity * delta;

func shoot(instigator, shoot_vel:Vector2, damage_override = null):
	self.instigator = instigator;
	sprite_2d.rotation = shoot_vel.angle() + PI/2;
