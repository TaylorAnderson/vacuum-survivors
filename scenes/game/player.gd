...
@onready var level_up_anim = $LevelUpAnim

@onready var level_up_sound = $LevelUpSound
@onready var level_up_sound_2 = $LevelUpSound2

# These can be upgraded at the shop.
@export var run_speed:float = 55;
@export var walk_speed:float = 45;
var element_effectiveness = 1;
...
signal finished_levelup_anim(new_level);
signal shot_bullet();
...
func _physics_process(delta):
	...
	if Input.is_action_just_pressed("shoot") and vacuum_bar.value >= shot_cost:
		shot_queued = true;
...
func shoot_bullet():
	...
	dust_bullet.shoot(self, shoot_angle * shot_speed, damage);
...
func _on_xp_bar_levelled_up(new_level):
	...
	level_up_anim.visible = true;
	level_up_anim.play("default");
	level_up_sound.play();
	await get_tree().create_timer(0.56).timeout
	level_up_sound_2.play();
	
	await level_up_anim.animation_finished
	level_up_anim.visible = false;
	finished_levelup_anim.emit(new_level);
	make_invincible();
